# Connect agent-activity to Microsoft Teams

`azd deploy` already did the Azure side for you:

- Azure Bot: created by `bot-service.bicep` (Microsoft Teams channel enabled)
- Bot ID (msaAppId): the agent's `instance_identity.client_id`. Fetch it with:

  ```powershell
  az rest --method get `
    --url "<project-endpoint>/agents/agent-activity?api-version=v1" `
    --resource https://ai.azure.com `
    --query instance_identity.client_id -o tsv
  ```

  (Referred to below as `<BOT_ID>`.)
...
3. Left menu **App features** -> **Bot** -> **Select an existing bot** -> paste
   your `<BOT_ID>`, tick the **Personal** scope, then **Save**.
...
```json
{
  ...
  "name": { "short": "agent-activity", "full": "agent-activity" },
  "description": { "short": "agent-activity agent", "full": "agent-activity agent on Microsoft Teams" },
...
```sh
zip -j agent-activity-teams-app.zip manifest.json color.png outline.png          # bash
```
```powershell
Compress-Archive manifest.json,color.png,outline.png agent-activity-teams-app.zip # PowerShell
```
...
atk install --file-path agent-activity-teams-app.zip --scope Personal
```
